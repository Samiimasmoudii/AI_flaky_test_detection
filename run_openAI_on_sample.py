...
from inc.processor_openai import run_llm_on_before_file, compare_fixes, MODEL_NAME
...
def process_result_folder(folder_path):
    ...
    try:
        ...
        # Run OpenAI analysis on the before file
...
def main():
    ...
    logging.info(f"Found {len(test_cases_to_process)} test cases to process from Excel file")
    ...
    print(f"Total number of files: {len(files_to_process)}")
    print("\nFiles list:")
    for idx, file_info in enumerate(files_to_process, 1):
        print(f"{idx}. Category: {file_info['category']}")
        print(f"   Test Case: {file_info['test_case']}")
    ...
    for file_info in files_to_process:
        logging.info(f"Processing test case: {file_info['test_case']}")
        process_result_folder(file_info['path'].parent)
